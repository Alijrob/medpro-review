"""
test_fl_courts.py -- Florida eCourts adapter tests (Phase 3-C).

Exercises FlCourtsConnector against stub transports (no live network).
Tests cover: config identity, contract harness, offset/limit pagination,
short-page termination, camelCase normalization, dict-wrapped response
unwrapping (cases/results/data keys), and failure modes.

Run:
    PYTHONPATH=src pytest tests/connectors/test_fl_courts.py -v
"""
...
from connectors.sources.court_records import FlCourtsConnector, fl_courts_config
...
def _fl_row(**over: Any) -> dict[str, Any]:
    row: dict[str, Any] = {
        "case_number": "2022-CA-001234",
        "case_style": "Smith v. Florida Medical Center",
...
def _fl_row_camel(**over: Any) -> dict[str, Any]:
    """Row using camelCase API field names."""
    row: dict[str, Any] = {
        "caseNumber": "2021-CA-005678",
        "caseStyle": "Doe v. Miami Hospital LLC",
        "courtName": "Miami-Dade County Circuit Court",
        "dateFiled": "2021-08-20",
        "caseType": "CC",
        "caseStatus": "Closed",
    }
    row.update(over)
...
def _wrapped_page(rows: list[dict[str, Any]], key: str = "cases") -> StubResponse:
...
# ---------------------------------------------------------------------------
class TestConfig:
    def test_identity_is_court_fl(self):
        cfg = fl_courts_config()
        assert cfg.source_id == "court_fl"
        assert cfg.source_name == "Florida eCourts"
        assert cfg.source_category is SourceCategory.COURT
        assert cfg.integration_method is IntegrationMethod.REST_API

    def test_base_url_points_to_flcourts(self):
        cfg = fl_courts_config()
        assert "flcourts.gov" in cfg.base_url

    def test_overrides_apply(self):
        cfg = fl_courts_config(rate_limit_per_sec=1.0)
...
class TestContractHarness:
    ...
    def test_run_reports_healthy_on_success(self):
        conn = FlCourtsConnector(
            fl_courts_config(),
            transport=stub_transport(_page([_fl_row(), _fl_row(case_number="2022-CA-999999")])),
        )
    ...
    def test_source_id_propagated_to_records(self):
        ...
        result = asyncio.run(conn.run())
        assert all(r.source_id == "court_fl" for r in result.records)
...
class TestOffsetPagination:
    def test_pages_via_offset_until_short_page(self):
        transport, calls = _recording_transport(
            _page([_fl_row(case_number="1"), _fl_row(case_number="2")]),
            _page([_fl_row(case_number="3")]),
        )
        conn = FlCourtsConnector(fl_courts_config(), transport=transport, page_size=2)
        result = asyncio.run(conn.run())
    ...
    def test_empty_array_terminates_pagination(self):
        transport, calls = _recording_transport(
            _page([_fl_row(case_number="A"), _fl_row(case_number="B")]),
            _page([]),
        )
        conn = FlCourtsConnector(fl_courts_config(), transport=transport, page_size=2)
        result = asyncio.run(conn.run())
    ...
    def test_party_name_sent_in_params(self):
        ...
        assert calls[0]["params"]["partyName"] == "Doe, Jane"
        assert calls[0]["params"]["limit"] == 75
...
class TestFieldNormalization:
    def test_camel_case_fields_normalized(self):
        conn = FlCourtsConnector(
            fl_courts_config(),
            transport=stub_transport(_page([_fl_row_camel()])),
        )
        result = asyncio.run(conn.run())
        assert result.status is FetchStatus.SUCCESS, result.errors
        raw = result.records[0].raw
        for field in ("case_number", "case_style", "court",
    ...
    def test_dict_wrapped_response_cases_key_unwrapped(self):
        conn = FlCourtsConnector(
            fl_courts_config(),
            transport=stub_transport(_wrapped_page([_fl_row()], key="cases")),
        )
        result = asyncio.run(conn.run())
        assert result.status is FetchStatus.SUCCESS
    ...
    def test_dict_wrapped_response_results_key_unwrapped(self):
        conn = FlCourtsConnector(
            fl_courts_config(),
            transport=stub_transport(_wrapped_page([_fl_row()], key="results")),
        )
        result = asyncio.run(conn.run())
        assert result.status is FetchStatus.SUCCESS
