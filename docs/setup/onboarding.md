...
**Phase 3-E COMPLETE** -- Review Platform Adapters (Google Places, Yelp Fusion). New `src/connectors/sources/review_platforms/` subpackage: `GooglePlacesConnector` (cursor pagination via `next_page_token`; API key as query param; 6-field contract: `place_id`/`name`/`rating`/`user_ratings_total`/`formatted_address`/`reviews`; reviews defaults to `[]`) and `YelpConnector` (offset/limit pagination; Bearer token auth; 1000-result hard cap via `_YELP_MAX_OFFSET`; 6-field contract: `id`/`name`/`rating`/`review_count`/`location`/`categories`; None location coerces to `{}`, categories to `[]`). Both use `SourceCategory.REVIEW_PLATFORM`. Migration 0011 seeds both rows. 47 new tests; 1790 total passing. DECISIONS.md Entry 041. **Phase 6-A (Docker Compose stack on Hostinger) is next.**
