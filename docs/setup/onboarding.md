...
**Initial validation deploy: Hostinger VPS (147.93.119.147)** -- Phase 6. Docker Compose for Postgres + Redis; PM2 for FastAPI backends + Next.js; Nginx vhost for researchyourdoctor.com; Auth0 + Stripe live credentials. EKS/Terraform architecture (Phase 1-B, 1-E) is retained as code and deferred to Phase 7 until the product is validated with paying users. See DECISIONS.md Entry 042.

---

## Current Phase

**Phase 6-B COMPLETE** -- PM2 ecosystem config written. `ecosystem.config.js` defines 8 processes: 7 FastAPI backends via Poetry/uvicorn (auth:8000, gateway:8080, audit:8001, monitor:8002, search:8003, reports:8004, payments:8005) + Next.js frontend (port 3100). `scripts/pm2-start.sh` sources `.env.hostinger`, builds Next.js, and starts/restarts all processes. Temporal worker intentionally excluded (Temporal server not yet installed on Hostinger). Services without their external dep (OpenSearch, Temporal, Auth0, Stripe) boot safely and return not-ready on /readyz. To deploy on Hostinger: `git pull && nano .env.hostinger && bash scripts/pm2-start.sh start && pm2 startup && pm2 save`. **Phase 6-C (Nginx vhost + SSL for researchyourdoctor.com) is next.**
**Phase 6-A COMPLETE** -- Data stores live on Hostinger (147.93.119.147). Native Postgres 16 used instead of Docker (existing server already ran Postgres with 13 app databases; DECISIONS.md Entry 043). Created medpro_admin role (CREATEROLE + CREATEDB), medpro + medpro_audit databases, uuid-ossp + pg_trgm extensions. Redis 7 installed via apt; bound to 127.0.0.1:6379 with password auth, 256MB maxmemory, allkeys-lru, systemd enabled. Ran `poetry install --without dev` on Hostinger; `alembic upgrade head` applied all 11 migrations (fixed 6 bug categories in migration files -- see DECISIONS.md Entry 043). 13 tables + 33 source health seeds in medpro DB. Alembic at revision 0011. 105 data/migration tests still passing locally. Server credentials in `/root/medpro-review/.env.hostinger` (not committed).
**Phase 3-E COMPLETE** -- Review Platform Adapters (Google Places, Yelp Fusion). New `src/connectors/sources/review_platforms/` subpackage: `GooglePlacesConnector` (cursor pagination via `next_page_token`; API key as query param; 6-field contract: `place_id`/`name`/`rating`/`user_ratings_total`/`formatted_address`/`reviews`; reviews defaults to `[]`) and `YelpConnector` (offset/limit pagination; Bearer token auth; 1000-result hard cap via `_YELP_MAX_OFFSET`; 6-field contract: `id`/`name`/`rating`/`review_count`/`location`/`categories`; None location coerces to `{}`, categories to `[]`). Both use `SourceCategory.REVIEW_PLATFORM`. Migration 0011 seeds both rows. 47 new tests; 1790 total passing. DECISIONS.md Entry 041.
**Phase 4-H COMPLETE** -- Multi-Model AI Report Intelligence Layer. New `src/ai/` package: `AISettings` (env prefix AI_, all keys default None for graceful degradation), `NarrativeResult`/`NarrativeSection` Pydantic models (JSON-serialisable/Temporal-safe), recursive PII scrubber (`scrub_pii()` strips home_address/personal_phone/personal_email/dob/ssn before any prompt), `BaseAIProvider` ABC, `GeminiProvider` (google-genai SDK, injectable client), `AnthropicProvider` (anthropic SDK, injectable client), prompt builders (research/analysis/format), `NarrativeGenerator` 3-step pipeline, `generate_narrative()` router. Model routing: Gemini 2.5 Pro (research, 1M context sweep) -> Claude Opus (risk analysis + consumer summary, one coherent call) -> Claude Haiku (HTML formatting). New async Temporal activity `generate_ai_narrative_activity` (Step 5.5, best-effort, `_BEST_EFFORT_RETRY`). `GenerateNarrativeInput`/`GenerateNarrativeOutput` added to worker models. `GenerateReportInput.narrative` field added (optional). `render_html()` now accepts `narrative: NarrativeResult | None`. HTML template: new AI Narrative section with risk-level CSS classes. `narrative_activity_timeout_s=120` in WorkerSettings. `anthropic ^0.40` + `google-genai ^1.0` added to pyproject.toml. `ai` package added to packages list. 90 new tests (1759 total, 18 skipped). DECISIONS.md Entry 040. Feature flag: `AI_NARRATIVE_ENABLED=false` bypasses step entirely. **Phase 3-E (Review Platform Adapters) is next.**
**Phase 3-D COMPLETE** -- Commercial Data Adapters (Ribbon Health, Healthgrades, Vitals). 3 new `SourceConnector` subclasses in `src/connectors/sources/commercial/`: `RibbonHealthConnector` (T3, REST_API page-number/total_pages, `Authorization: Token`), `HealthgradesConnector` (T4, REST_API offset/limit short-page sentinel, `Authorization: Bearer`), `VitalsConnector` (T4, REST_API offset/limit, Bearer). All use `SourceCategory.COMMERCIAL_DIRECTORY`. All raise `AuthenticationError` if `api_key` absent (visible in FetchResult.errors). Ribbon: 6-field contract with `list` type for `locations`/`insurances`/`affiliations`; `specialties` list coerced to str. Healthgrades: numeric `rating`/`review_count` coerced to str; None `board_certifications` to `[]`. Vitals: same numeric coercion; None `education` to `[]`; `training` alias. Migration 0010 seeds 3 `commercial_directory` rows (ribbon_health/healthgrades/vitals); chains from 0009. `commercial/__init__.py` + `sources/__init__.py` updated. 74 new tests (1669 total, 18 skipped). DECISIONS.md Entry 039. **Phase 3-E (Review Platform Adapters: Google Places, Yelp) is next.**
...
| `src/ai/` | **Phase 4-H multi-model AI narrative layer.** `config.py` (AISettings, env prefix AI_), `models.py` (NarrativeResult/NarrativeSection), `pii.py` (scrub_pii -- strips 5 PII fields recursively), `providers/` (BaseAIProvider ABC; GeminiProvider via google-genai; AnthropicProvider via anthropic SDK; all injectable for tests), `prompts/` (build_research_prompt/build_analysis_prompt+parse_analysis_response/build_format_prompt), `narrative.py` (NarrativeGenerator 3-step orchestrator), `router.py` (generate_narrative() top-level fn). Feature flag: AI_NARRATIVE_ENABLED. |
| `src/workers/activities/generate_ai_narrative.py` | **Phase 4-H async Temporal activity.** generate_ai_narrative_activity -- best-effort (never raises); calls generate_narrative(); returns GenerateNarrativeOutput with serialised NarrativeResult or None. Registered with _BEST_EFFORT_RETRY in ProviderPipelineWorkflow Step 5.5. |
| `src/connectors/sources/commercial/` | **Phase 3-D commercial data adapters (D1-D3, T3/T4 licensed)** -- 3 adapters, all `SourceCategory.COMMERCIAL_DIRECTORY`. Ribbon Health (D1/T3): page-number/total_pages, `Authorization: Token`, list-type contract fields (locations/insurances/affiliations), specialties-list-to-str coercion. Healthgrades (D2/T4): offset/limit short-page sentinel, `Authorization: Bearer`, numeric rating/review_count coerced to str, board_certifications list. Vitals (D3/T4): same offset/limit pattern, education list, training alias. All raise `AuthenticationError` (visible in `run()` result) if api_key absent. All base URLs are placeholders. Tested stub-only; live ingest requires signed license AND Phase 0 gate. |
| `src/data/migrations/versions/0010_commercial_source_seeds.py` | Migration 0010 -- seeds 3 Phase 3-D `source_health_records` rows (ribbon_health/healthgrades/vitals, category='commercial_directory'). Chains from 0009. ON CONFLICT DO NOTHING. Does not touch court or state board rows. |
